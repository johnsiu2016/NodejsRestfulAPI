...
app.use(expressValidator({
    customValidators: {
        arrayIsIn: function arrayIsIn(providedArray, checkingArray) {
            if (checkingArray.length === 0) return false;
            var filtedArray = providedArray.filter(function (providedValue) {
...
api.use(function (req, res, next) {
    var apikey = req.query.apikey || req.body.apikey || req.headers.apikey;
    if (apikey === process.env.APIKEY) next();else res.json(myUtil.apiOutputTemplate("error", "Unauthorized: API key is not correct"));
});
// account, User Model
